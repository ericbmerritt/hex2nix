...
remove_unbuildable_deps(Deps) ->
    lists:filter(fun unbuildable_dep/1, Deps).

-spec unbuildable_dep(h2n_fetcher:dep_desc()) -> boolean().
unbuildable_dep(#dep_desc{deps = Deps,
...
try_build(#dep_desc{app = App}) ->
    ...
    case h2n_util:run("nix-build '<nixpkgs>' -A beamPackages.~s", [NixName]) of
        {ok, _} ->
            true;
        {error, Status, Out} ->
